...
class PieChart
{
  ...
  void draw()
  {
    ...
    if (!closed)
    {
      ...
      fill(BLACK);
      text("Percentage of "+(lateness?"Delayed ":"On Time ")+"Flights " + (destAirport ? "to ":"from ") + stateLabel + ": ( 01/" + ((startDate < 10) ? "0"+startDate : startDate) + "/2022 - 01/" + ((endDate < 10) ? "0"+endDate : endDate) + "/2022 )", width/2, topMargin - 150);
